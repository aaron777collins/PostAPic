<?php
    ...
    // verify it is set
    if (!isset($_POST["useridToDelete"]) || !isset($_POST["userid"]) || !isset($_POST["token"])) {
        $response = array(
    ...
    // verify not empty
    if (empty($useridToDelete) || empty($userid) || empty($token)) {
        $response = array(
            "error" => "Please fill in all required fields."
        );
        echo json_encode($response);
    ...
    if ($result->num_rows > 0) {
        ...
        if ($userid === $useridToDelete || $row["username"] === "admin") {
